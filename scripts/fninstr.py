...
all_fns = set()
fns_passed_as_args = {}
for name in list(prots.keys()):
    all_fns.add(name)
print("%d fn names in prots" % (len(all_fns)))
for name in list(fundefs.keys()):
    all_fns.add(name)
print("%d fn names in prots+fundefs" % (len(all_fns)))
for name in list(calls.keys()):
    all_fns.add(name)
...
instr = set()
for name in list(instr_judgement.keys()):
...
while any_change:
    any_change = False
    for called_fn_name in list(calls.keys()):
