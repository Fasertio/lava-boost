...
def competition_bugs_and_non_bugs(limit, db, allowed_bugtypes, buglist):
    ...
    def parse(item):
        ...
        if not (dfl in list(dfl_fileline.keys())): dfl_fileline[dfl] = 0
        if not (afl in list(afl_fileline.keys())): afl_fileline[afl] = 0
    ...
    if buglist is None:
        ...
        while True:
            ...
            if sum([len(x) for x in list(atp_item_lists.values())]) == 0:
    ...
    for atp, count in list(afls.items()):
...
def main():
    ...
    with open(log_build_sh, "w") as build:
        ...
        """.format(
            bugs_build=bd,
            make_clean = project["clean"] if "clean" in list(project.keys()) else "",
            configure=project['configure'] if "configure" in list(project.keys()) else "",
            log_make = log_make,
            internal_builddir = internal_builddir,
            install = project['install'].format(install_dir=lava_installdir),
            post_install = project['post_install'] if 'post_install' in list(project.keys()) else "",
            ))
