...
USAGE() {
  ...
  echo "   -e | --heap Implementa heap overflow"
...
RESET_DB() {
    ...
    if [ $ho -eq 1 ] ; then
        run_remote "$pandahost" "psql -d $db -f $lava/tools_ho/lavaODB/generated/lava.sql -U postgres" "$lf"
    else
...
if [ $taint -eq 1 ]; then
    ...
    do
        ...
        run_remote "$pandahost" "$python3 $scripts/bug_mining.py $hostjson $project_name $input $curtail $HO" "$lf"
...
if [ $inject -eq 1 ]; then
    ...
    do
        ...
        run_remote "$testinghost" "$python3 $scripts/inject.py -t $bugtypes -m $many -ho $ho -e $exitCode $kt $fix $hostjson $project_name" "$lf"
