...
def run_cmd(cmd, envv=None, timeout=30, cwd=None, rr=False, shell=False):
    if type(cmd) in [str, str] and not shell:
        cmd = shlex.split(cmd)
...
def mutfile(filename, fuzz_labels_list, new_filename, bug,
    ...
    file_bytes = bytearray(open(filename,"rb").read())
    ...
    if bug.type == Bug.REL_WRITE:
        ...
        for (i, offset) in zip(list(range(4)), fuzz_labels_list[0]):
            file_bytes[offset] = a_val[i]

        for (i, offset) in zip(list(range(4)), fuzz_labels_list[1]):
            file_bytes[offset] = b_val[i]

        for (i, offset) in zip(list(range(4)), fuzz_labels_list[2]):
            file_bytes[offset] = c_val[i]

    else:
        for fuzz_labels in fuzz_labels_list:
            for (i, offset) in zip(list(range(4)), fuzz_labels):
...
def run_lavatool(bug_list, lp, host_file, project, llvm_src, filename,
    ...
    val_for_bytes = ret[1][0]
    if isinstance(ret[1][0], bytes):
        val_for_bytes = ret[1][0].decode("utf-8")

    for line in val_for_bytes.split("\n"):
    ...
    def __init__(self, project):
        ...
        if 'source_root' in project:
            ...
            tar_files = subprocess32.check_output(['tar', 'tf',
                                                   project['tarfile']],
                                                  stderr=sys.stderr)
            self.source_root = tar_files.decode("utf-8").splitlines()[0].split(os.path.sep)[0]
...
def inject_bugs(bug_list, db, lp, host_file, project, args,
    ...
    if not os.path.exists(join(lp.bugs_build, 'config.log')) \
            and 'configure' in list(project.keys()):
...
# find llvm_src dir so we can figure out where clang #includes are for btrace
    ...
    for src_dir in src_dirs:
        ...
        if isinstance(lp.bugs_build, bytes):
            lp.bugs_build = lp.bugs_build.decode("utf-8")
        if isinstance(src_dir, bytes):
    ...
    if "injfixupsscript" in list(project.keys()):
    ...
    if 'arm' in project and project['arm']:
        print("make arm")
        check_call(project["make_arm"], cwd=lp.bugs_build, shell=True)
        # check_call(shlex.split("ls -l "+lp.bugs_build))   
        print("install arm")         
        check_call(project['install_arm'].format(install_dir="lava-install"), cwd=lp.bugs_build, shell=True)
               
    if 'post_install' in list(project.keys()):
        check_call(project['post_install'], cwd=lp.bugs_build, shell=True)

    val_0 = outp[0]
    val_1 = outp[1]
    if isinstance(outp[0], bytes):
        val_0 = outp[0].decode("utf-8")
    if isinstance(outp[1], bytes):
        val_1 = outp[1].decode("utf-8")    
        
    build = Build(compile=(rv == 0), output=(val_0 + ";" + val_1),
                  bugs=bugs_to_inject)
...
def validate_bug(db, lp, project, bug, bug_index, build, args, update_db,
    ...
    if update_db:
        ...
        db.session.add(Run(build=build, fuzzed=bug, exitcode=rv,
                           output=(val_0 + '\n' + val_1).encode("ascii","ignore").decode('ascii', 'ignore'),
...
def validate_bugs(bug_list, db, lp, project, input_files, build,
    ...
    for input_file in input_files:
        ...
        if update_db:
            db.session.add(Run(build=build, fuzzed=None, exitcode=rv,
                               output=lines.encode("ascii","ignore").decode('ascii', 'ignore'),
    ...
    for bug_index, bug in enumerate(bugs_to_inject):
        ...
        if bug_solutions and bug.id in list(bug_solutions.keys()):
    ...
    if len(bugs_to_inject) > 0:
        ...
        print("yield {:.2f} ({} out of {}) real bugs (95% CI +/- {:.2f}) "
