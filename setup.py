...
def is_package_installed(pkg):
    ...
    if command_exited_nonzero("dpkg -s {}".format(pkg)):
        ...
        try:
            ...
            return not command_exited_nonzero("python3 -c \"import {}\""
                                              .format(python_pkg))
...
PANDA_DIR = abspath(join(LAVA_DIR, "panda/panda"))
...
LAVA_DEPS = ["libjsoncpp-dev", "postgresql", "jq",
             ...
             "build-essential", "python3-pip", "libprotobuf-dev", "libfdt-dev", "gcc-9","llvm-11"]
...
def main():
    ...
    run("sudo apt install libepoxy-dev")
    run("sudo apt install libepoxy-dev:i386")
    progress("Checking for PANDA in " + PANDA_DIR)
    if not isdir(PANDA_DIR):
        os.chdir(dirname(PANDA_DIR))
        if not isfile("install_ubuntu.sh"):
    ...
    run("sudo apt install gcc-9 gcc-9-plugin-dev")
    #install here odb 2.5 then apt install odb
    progress("Checking for ODB orm libraries")
    if not isfile('/tmp/build2-build/build2-install-0.15.0.sh'):
        if not isdir("/tmp/build2-build"):
            run("mkdir /tmp/build2-build")
            os.chdir("/tmp/build2-build")
            run("curl -sSfO https://download.build2.org/0.15.0/build2-install-0.15.0.sh")
            run("sh build2-install-0.15.0.sh")
        if not isdir("/tmp/odb-build"):
            run("mkdir /tmp/odb-build")
        os.chdir("/tmp/odb-build")
    
    if not isfile('/usr/bin/odb'):
        run("bpkg create -d odb-gcc-9 cc config.cxx=g++ config.cc.coptions=-O3 config.bin.rpath=/usr/lib config.install.root=/usr/ config.install.sudo=sudo")
        os.chdir("/tmp/odb-build/odb-gcc-9")
        run("bpkg build odb@https://pkg.cppget.org/1/beta")
        run("bpkg install odb")
    
    if not isfile('/usr/local/lib/libodb.so') and not isfile('/usr/lib/libodb.so') :
        os.chdir("/tmp/odb-build")
        run("bpkg create -d libodb-gcc-9 cc config.cxx=g++ config.cc.coptions=-O3 config.install.root=/usr/ config.install.sudo=sudo")
        os.chdir("/tmp/odb-build/libodb-gcc-9")
        run("bpkg add https://pkg.cppget.org/1/beta")
        run("bpkg fetch")
        run("bpkg build libodb")
        run("bpkg build libodb-pgsql") 
        run("bpkg build libodb-qt")
        run("bpkg build libodb-boost")
    ...
    if not isfile('/usr/local/lib/libodb-pgsql.so') and \
        ...
        run("sudo make install")
    """
    run("sudo apt install -y odb libodb-pgsql-dev")
    progress("Finished installing ODB libraries")

    progress("Installing python dependencies.")
    if command_exited_nonzero("python3 -c \"import {}\"".format("subprocess32")):
        run("sudo pip3 install subprocess32")
    ...
    run("cp expect.py ./panda/panda/panda/scripts/")
    return 0
